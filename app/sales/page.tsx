...
export default function SalesPage() {
  ...
  const onEditStock = (id: string) => {
    router.push('/record-sale', { state: { id } } as any)
  }

  const handleMultiSales = () => {
    router.push('/batch-sale')
  }
        ...
        <section className="bg-[#edf0f0b9] min-h-screen">
          <div className="container m-auto">
            <div className="grid grid-cols-1 gap-6">
              ...
              <div>
                <h2 className="text-xl font-semibold mb-4 ml-4">Inventory Management</h2>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-100">
                        <tr>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">No</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Product Image</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">ID</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Category</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Product Name</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Price</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Stock</th>
                          <th className="px-4 py-3 text-xs font-medium text-gray-500">Restock Level</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Unit</th>
                          <th className="px-4 py-3 text-xs text-left font-medium text-gray-500">Action</th>
                        </tr>
                      </thead>
                        ...
                        {stocks.map((stock: StockItem, index: number) => (
                          <tr key={stock.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                              <img 
                                className="size-10" 
                                src={stock.Product_image ? `http://localhost:5000/uploads/${stock.Product_image}` : '/placeholder.png'} 
                                alt="Stock Image" 
                              />
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
